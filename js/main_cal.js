// global variable for storing loaded csv data
var csv_data;
...
function parseCalendarData(type) {
    ...
    for (row in csv_data) {
        ...
        if (type == 0) {
            if (data[csv_data[row].Date_start]) 
                data[csv_data[row].Date_start] += 1;
            else
                data[csv_data[row].Date_start] = 1;
        }
        ...
        if (type == 1) {
            if (data[csv_data[row].Date_end]) 
                data[csv_data[row].Date_end] += 1;
            else
                data[csv_data[row].Date_end] = 1;
        }
    }
...
function renderCalendar(type) {
    ...
    for (var i = calendars.length - 1; i >= 0; i--) 
        document.body.removeChild(calendars[i]);
    ...
    function loadData() {
        ...
        rect.filter(function (d) {return d in data;})
            .attr("class", function (d) {return "day " + color(Math.log(data[d]));})
            .select("title")
    ...
    if (!csv_data) {
        d3.csv("/data/calendar_data.csv", function (error, csv) {
    ...
    // load data into graph if it is already downloaded
    else {   
        loadData();
    }
